...
ANNOT_PATH = os.path.join(dirname(dirname(dirname(dirname(__file__)))), os.path.join('data', 'h36m/dict_h36m_desc.pkl'))
...
KEYPOINTS_WORLD_PATH = '/cvlabdata2/home/davydov/hpe_project/data/h36m/dict_h36m_keypoints_world.pkl'
with open(KEYPOINTS_WORLD_PATH, 'rb') as f:
    DICT_H36M_KEYPOINTS_WORLD = pickle.load(f)

CAMERA_PARAMS_PATH = '/cvlabdata2/home/davydov/hpe_project/data/h36m/camera-parameters.json'
...
def _check_poses(ds):
    ...
    from h36m_visual_utils import plot_pose2d, plot_pose3d
    rows, cols = 10, 10
    fig_2d, ax_2d = plt.subplots(rows, cols, figsize=(4*cols,4*rows))
    ...
    for i in range(rows):
        for j in range(cols):
            ...
            ### img
            img = sample['img']
            ax_2d[i,j].imshow(img)
            ax_2d[i,j].set_axis_off()

            ### pose2d
            pose2d = sample['pose2d']
            pose_mask = sample['pose_mask']
            plot_pose2d(ax_2d[i,j], pose2d, mask=pose_mask, skeleton_bones=BONES_H36M_3D_17, annotate=annotate)
    ...
    for fig, name in zip([fig_2d, fig_3d], ['2d', '3d']):
        fig.subplots_adjust(wspace=0.01, hspace=0.01)
        fig.patch.set_facecolor('white')
        fig.tight_layout()
        fig.savefig(f'./SAMPLE_h36m_{name}.png')
