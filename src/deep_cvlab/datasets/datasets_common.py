...
def get_datasets_by_config(cfg):
    assert len(cfg.DATASETS) > 0, 'At least one dataset must be specified for training!'
    datasets = edict()
...
def get_dataloaders_by_config(cfg, datasets, num_gpus=1):
    ...
    for dset in cfg.DATALOAD:
        ...
        params = dl_dict.PARAMS if 'PARAMS' in dl_dict else {}
        params['batch_size'] = params.batch_size * num_gpus if 'batch_size' in params else 32 # custom default
