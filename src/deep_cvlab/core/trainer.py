...
from ..losses import losses_common
from ..optimizers import optimizers_common
from ..datasets import datasets_common
from ..procedures import procedures_common
...
class Trainer():
    ...
    def __init__(self, cfg): # initialized by <configname>.yaml file
        ...
        self.device0 = th.device(f'cuda:{self.gpus[0]}')
        ...
        self.losses, self.losses_weights = losses_common.get_losses_by_config(self.cfg, self.device0)

        self.logger.info('=> Trainer init: loading optimizers and schedulers...')
        self.optim = optimizers_common.get_optim_by_config(self.cfg, self.models)
        self.sched = optimizers_common.get_sched_by_config(self.cfg, self.optim)
        ...
        self.meters = metrics.get_meters_by_config(self.cfg, self.losses.keys())

        self.load_state(self.cfg)

        self.logger.info('=> Trainer init: loading datasets...')
        self.datasets = datasets_common.get_datasets_by_config(self.cfg)
        self.dataload = datasets_common.get_dataloaders_by_config(self.cfg, self.datasets, num_gpus=len(self.gpus))

        self.proc = procedures_common.get_procedures_by_config(self.cfg)
    ...
    def save_state(self, best=False):
        ...
        th.save(state2save, joinpath(self.final_output_dir, 'ckpt.pth')) 
        ...
        if self.cfg.SAVE_EVERY_EPOCH and self.cur_epoch % self.cfg.SAVE_EVERY_EPOCH == 0: 
            th.save(state2save, joinpath(self.final_output_dir, f'ckpt_{self.cur_epoch:04d}.pth'))
        if best:
            self.logger.info(f'=> Epoch [{self.cur_epoch}] - THE BEST MODEL SO FAR...\n')
            th.save(state2save, joinpath(self.final_output_dir, 'best.pth'))
    ...
    def load_checkpoint(self, ckpt):
        if isinstance(ckpt, str):
            ckpt_dict = th.load(ckpt, map_location='cpu')
        
        elif isinstance(ckpt, list):
            # NOTE all state dicts are combined together
            ckpt_dict = {}
            for ckpt_path in ckpt:
                ckpt_dict.update(th.load(ckpt_path, map_location='cpu'))  
            ...
            for model, ckpt_path in ckpt.items(): # ckpt keys must coincide with all initialized models
                ...
                model_dict = th.load(ckpt_path, map_location='cpu')
                # one needs to check whether "model" and "model_ckpt" (from the <model_ckpt>_state_dict) 
                # coincide
    ...
    def run(self):
        ...
        self.logger.info(f'\nThe best performance for validation loss is "{self.perf_indicator_best}" \n')