...
def status_msg_acai(trainer, batch_idx, dl_len, lossmeter, lossmeter1, lossmeter2):
    '''
    Provides a verbose status of training, what epoch is, what batch is, what current metrics value is
    '''
    if batch_idx % trainer.print_freq == 0 or batch_idx == dl_len or batch_idx == 1:
        # print every first, last and chosen ("print_freq") batches
        msg = (
          f'\nAutoencoder Loss: {lossmeter.cur_val:12.5e}, avg {lossmeter.cur_avg:12.5e}\t'
          f'Reconstruction Loss: {lossmeter1.cur_val:12.5e}, avg {lossmeter1.cur_avg:12.5e}\t'
          f'Critic Fooling Loss: {lossmeter2.cur_val:12.5e}, avg {lossmeter2.cur_avg:12.5e}\n'
              )
        trainer.logger.info(msg)
