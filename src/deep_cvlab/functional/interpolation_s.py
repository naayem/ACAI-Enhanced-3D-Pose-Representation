...
def slerp(x, y, steps=10, slerp_type='in'):
    ...
    theta = th.acos(cos_theta).repeat(1,steps).view(B,-1,1) # B x steps x 1
    
    interp_steps = th.linspace(0, 1, steps).repeat(B,1).view(B,-1,1) # B x steps x 1
    ...
    alpha = th.sin(theta - phi) / sin_theta # B x steps x 1
    beta = th.sin(phi) / sin_theta # B x steps x 1
    z = alpha * x.view(B,1,-1) + beta * y.view(B,1,-1) # B x steps x D
    return z

def linear(x, y, steps=10):
    '''
    Creates a tensor of size `steps` whose values are evenly spaced in a euclidian space
    between `x` and `y`, inclusive.
    Interpolation is done for pairs of `x` and `y` vectors in the batch.
    
    z_i := interp(x_i, y_i, steps)

    Args:
        x (tensor, B x D): the starting value for the set of points
        y (tensor, B x D): the ending value for the set of points
        steps (int): number of interpolated points vectors
        
    Return:
        out (tensor, B x steps x D): interpolations between all pairs of `x`/`y` vectors
    '''
    assert x.size(0) == y.size(0)
    B = x.size(0)
    
    interp_steps = th.linspace(0,1,steps).repeat(B,1).view(B,-1,1) # B x steps x 1
    z = (1 - interp_steps) * x.view(B,1,-1) + interp_steps * y.view(B,1,-1) # B x steps x D
    return z
...
def create_sequence(x, y, steps, seq_type='S', slerp_type='in'):
    ...
    if seq_type == 'S':
        ...
        thetas = th.acos(cos_thetas) # B x 1
...
def get_interpolation(x, y, steps=10, latent_space_type='S', **kwargs):
    if latent_space_type == 'S':
        ...
        return linear(x, y, steps=steps, **kwargs)

def random_sinterpolation(x, y):
    assert x.size(0) == y.size(0)
    # x -> BxD  and  y -> BxD
    B = x.size(0)
    
    cos_theta = (x * y).sum(dim=1, keepdim=True)
    theta = th.acos(cos_theta) # B x 1
    
    # Randomzie interpolated coefficient alpha and divide by two to get interval [0,0.5]
    alpha = 0.5 * th.rand((B),1).to(x.device)

    phi = theta * alpha # B x 1

    sin_theta = th.sin(theta)
    a = th.sin(theta - phi) / sin_theta # B x 1
    b = th.sin(phi) / sin_theta # B x 1
...
def plot_test_sequence(seq_type, steps=10, slerp_type='in', space_dim=2):
    ...
    fig.savefig(f'plot_TEEESTTTTT.png')
